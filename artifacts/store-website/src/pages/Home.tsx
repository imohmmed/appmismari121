...
function AppCard({ app, index }: { app: any, index: number }) {
  return (
    ...
    >
      <div className="relative w-28 h-28 rounded-[2rem] overflow-hidden bg-card border border-white/5 shadow-lg group-hover:shadow-primary/20 transition-all duration-300 group-hover:-translate-y-1">
        <img 
      ...
      <div className="text-center w-full">
        <h4 className="font-bold text-sm truncate px-1 text-foreground/90 group-hover:text-primary transition-colors">{app.name}</h4>
...
function AppSection({ title, icon: Icon, apps, loading }: { title: string, icon: any, apps: any[], loading: boolean }) {
  if (loading) return <div className="h-48 w-full animate-pulse bg-white/5 rounded-3xl mb-12"></div>;
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 mb-16">
      <div className="flex items-center justify-between mb-6">
        ...
        <button className="text-sm font-medium text-primary hover:text-accent transition-colors flex items-center gap-1">
          عرض الكل
        </button>
      </div>
  ...
  return (
    <PublicLayout>
      ...
      <section className="w-full min-h-[85vh] flex flex-col items-center justify-center relative px-4 text-center mt-[-80px] pt-[80px]">
        {/* Background image from requirements */}
        ...
        <motion.div 
          ...
          className="w-24 h-24 md:w-32 md:h-32 mb-8 rounded-[2.5rem] overflow-hidden shadow-2xl shadow-primary/30 relative"
        >
          <div className="absolute inset-0 bg-gradient-to-tr from-primary to-accent opacity-50 animate-pulse" />
        ...
        >
          <h1 className="text-5xl md:text-7xl font-black tracking-tight mb-6">
            متجر التطبيقات <span className="text-gradient">المميز</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="#plans" className="w-full sm:w-auto px-8 py-4 rounded-full text-lg font-bold bg-gradient-to-r from-primary to-accent text-white shadow-xl shadow-primary/25 hover:shadow-2xl hover:shadow-primary/40 hover:-translate-y-1 transition-all duration-300 flex items-center justify-center gap-2">
              <Sparkles className="w-5 h-5" />
              طلب اشتراك
            </a>
            <button className="w-full sm:w-auto px-8 py-4 rounded-full text-lg font-bold glass-button flex items-center justify-center gap-2">
        ...
        {/* Floating features */}
        <div className="mt-20 flex flex-wrap justify-center gap-6 max-w-4xl opacity-80">
          {[
            { icon: ShieldCheck, text: "ضمان تعويض" },
            { icon: Download, text: "تحديثات مستمرة" },
            { icon: Smartphone, text: "بدون جلبريك" },
          ].map((f, i) => (
            <div key={i} className="flex items-center gap-2 bg-white/5 rounded-full px-5 py-2 border border-white/10">
      ...
      <section id="plans" className="w-full py-24 relative">
        <div className="absolute inset-0 bg-primary/5 skew-y-[-3deg] transform origin-top-left -z-10" />
        
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-black mb-4">الاشتراكات</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {plansLoading ? (
              <>
                <div className="h-[500px] rounded-[3rem] animate-pulse bg-card/50" />
                <div className="h-[500px] rounded-[3rem] animate-pulse bg-card/50" />
              </>
            ) : plans.map((plan, i) => (
              <motion.div 
                ...
                className={`relative rounded-[3rem] p-8 md:p-10 transition-all duration-300 ${
                  plan.isPopular 
                    ? 'bg-gradient-to-b from-primary/20 to-card border-2 border-primary/50 shadow-2xl shadow-primary/20 scale-100 md:scale-105 z-10' 
                    : 'glass-panel'
                }`}
              >
                {plan.isPopular && (
                  <div className="absolute -top-5 inset-x-0 flex justify-center">
                    <span className="bg-gradient-to-r from-amber-400 to-yellow-600 text-black text-sm font-bold px-4 py-1.5 rounded-full shadow-lg flex items-center gap-1">
                      <Star className="w-4 h-4 fill-black" /> الأكثر طلباً
                    </span>
                  </div>
                )}
                
                <h3 className={`text-2xl font-bold mb-2 ${plan.isPopular ? 'text-gradient-gold' : ''}`}>
